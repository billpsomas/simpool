...
from timm.layers import PatchEmbed, Mlp, DropPath, SelectAdaptivePool2d, trunc_normal_, lecun_normal_, resample_patch_embed, \
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        """
        super().__init__()
        assert global_pool in ('', 'avg', 'token', 'simpool')
        assert class_token or global_pool != 'token'
        class_token = False if global_pool == 'simpool' else True
        ...
        self.gp = SelectAdaptivePool2d(dim=embed_dim, pool_type=self.global_pool, gamma=gamma, flatten=True)
