...
parser.add_argument('--dataset', metavar='NAME', default='torch/folder',
...
group.add_argument("--subset", default=-1, type=int, help="Number of images/class to use for training. If -1, all images are used.")
...
group.add_argument('--gp', default=None, type=str, choices=['avg', 'token', 'simpool'], metavar='POOL',
                   help='Global pool type, one of (fast, avg, max, avgmax, avgmaxc). Model default if None.')
group.add_argument('--gamma', default=None, type=utils.float_or_none,
...
def main():
    ...
    if utils.is_primary(args):
        if args.gp != 'token' and args.gp != 'avg':
            _logger.info(f'Model {safe_model_name(args.model)} with {args.gp} created, param count:{sum([m.numel() for m in model.parameters()])}')
        else:
    ...
    # TODO: Remember that I added this
    ...
    def imagenet_train_subset(dataset_train, subset):
    ...
    if (args.subset is not None) and (args.subset >= 1):
        dataset_train = imagenet_train_subset(dataset_train, args.subset)
