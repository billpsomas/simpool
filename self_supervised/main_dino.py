...
"""
Mostly copy-paste from DINO repo: https://github.com/facebookresearch/dino
"""
...
def get_args_parser():
    ...
    parser.add_argument('--arch', default='resnet50', type=str,
        choices=['vit_tiny', 'vit_small', 'vit_base', 'xcit', 'deit_tiny', 'deit_small', 'convnext_tiny', 'convnext_small', 'convnext_base', 'resnet18', 'resnet34', 'resnet50'] \
                + torchvision_archs,
    ...
    parser.add_argument('--out_dim', default=60000, type=int, help="""Dimensionality of
    ...
    parser.add_argument('--use_bn_in_head', default=True, type=utils.bool_flag,
        help="Whether to use batch normalizations in projection head (Default: False)") # TODO Maybe turn to True
    ...
    parser.add_argument('--use_fp16', type=utils.bool_flag, default=False, help="""Whether or not
        ...
        mixed precision if the loss is unstable, if reducing the patch size or if training with bigger ViTs.""") # TODO Maybe turn to True
    ...
    parser.add_argument("--lr", default=0.3, type=float, help="""Learning rate at the end of
    ...
    parser.add_argument('--global_crops_scale', type=float, nargs='+', default=(0.14, 1.0),
...
def train_dino(args):
    ...
    if args.arch == 'resnet50':
        ...
        teacher = vits.__dict__[args.arch](mode=args.mode, patch_size=args.patch_size)
    ...
    if args.mode == 'simpool':
        print(f"Student and Teacher are built: they are both {args.arch} network with SimPool.")
    else:
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch % args.eval_every == 0 or epoch == args.epochs - 1:
            knn_results = knn_evaluation_pipeline(
                teacher, dataset_train_knn, 
                data_loader_train_knn, dataset_val_knn, 
...
def knn_evaluation_pipeline(teacher, dataset_train_knn, data_loader_train_knn, dataset_val_knn, data_loader_val_knn, args):
    teacher.eval()

    # ============ extract features ... ============
    print("Extracting features for train set...")
    if 'convnext' in args.arch:
        train_features = extract_features(teacher.backbone, data_loader_train_knn, True)
    else:
        train_features = extract_features(teacher.module.backbone, data_loader_train_knn, True)
    print("Extracting features for val set...")
    if 'convnext' in args.arch:
        test_features = extract_features(teacher.backbone, data_loader_val_knn, True)
    else:
        test_features = extract_features(teacher.module.backbone, data_loader_val_knn, True)
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, data_loader,
    ...
    for it, (images, _) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            ...
            loss, entropy, KL_div = dino_loss(student_output, teacher_output, epoch)
...
class DINOLoss(nn.Module):
    ...
    def forward(self, student_output, teacher_output, epoch):
        ...
        total_KL_div /= n_loss_terms
        self.update_center(teacher_output)
        return total_loss, total_entropy, total_KL_div
