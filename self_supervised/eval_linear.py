...
def eval_linear(args):
    ...
    if args.mode == 'simpool':
        print(f"Model {args.arch} with SimPool is built.")
    else:
        print(f"Model {args.arch} is built.")
...
def train(model, linear_classifier, optimizer, loader, epoch, n, avgpool):
    ...
    for (inp, target) in metric_logger.log_every(loader, 20, header):
        ...
        # forward
        with torch.no_grad():
            if "vit" in args.arch and 'official' in args.mode:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--backend', default='nccl', type=str, help='Backend for CUDA run')
